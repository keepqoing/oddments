...
m_arFalseNorthing = [0.0, 600000.0, 600000.0]
...
src_m[GEO] = m_arMajor[GEO] * mlfn(e0fn(m_Es[GEO]), e1fn(m_Es[GEO]), e2fn(m_Es[GEO]), e3fn(m_Es[GEO]),
                                   m_arLatCenter[GEO])
dst_m[GEO] = m_arMajor[GEO] * mlfn(e0fn(m_Es[GEO]), e1fn(m_Es[GEO]), e2fn(m_Es[GEO]), e3fn(m_Es[GEO]),
                                   m_arLatCenter[GEO])
src_m[KATEC] = m_arMajor[KATEC] * mlfn(e0fn(m_Es[KATEC]), e1fn(m_Es[KATEC]), e2fn(m_Es[KATEC]), e3fn(m_Es[KATEC]),
                                       m_arLatCenter[KATEC])
dst_m[KATEC] = m_arMajor[KATEC] * mlfn(e0fn(m_Es[KATEC]), e1fn(m_Es[KATEC]), e2fn(m_Es[KATEC]), e3fn(m_Es[KATEC]),
                                       m_arLatCenter[KATEC])
...
def geo2tm(dsttype, in_pt, out_pt):
    ...
    ml = m_arMajor[dsttype] * mlfn(e0fn(m_Es[dsttype]), e1fn(m_Es[dsttype]), e2fn(m_Es[dsttype]), e3fn(m_Es[dsttype]),
                                   in_pt.getY())

    out_pt.setX(m_arScaleFactor[dsttype] * n * al * (1.0 + als / 6.0 * (
                1.0 - t + c + als / 20.0 * (5.0 - 18.0 * t + t * t + 72.0 * c - 58.0 * m_Esp[dsttype]))) +
                m_arFalseEasting[dsttype])
    out_pt.setY(m_arScaleFactor[dsttype] * (ml - dst_m[dsttype] + n * tq * (als * (0.5 + als / 24.0 * (
                5.0 - t + 9.0 * c + 4.0 * c * c + als / 30.0 * (
                    61.0 - 58.0 * t + t * t + 600.0 * c - 330.0 * m_Esp[dsttype]))))) + m_arFalseNorthing[dsttype])
...
def tm2geo(srctype, in_pt, out_pt):
    ...
    while (1):
        delta_Phi = ((con + e1fn(m_Es[srctype]) * math.sin(2.0 * phi) - e2fn(m_Es[srctype]) * math.sin(
            4.0 * phi) + e3fn(m_Es[srctype]) * math.sin(6.0 * phi)) / e0fn(m_Es[srctype])) - phi
    ...
    if (abs(phi) < (math.pi / 2)):
        ...
        ds = d * d
        out_pt.setY(phi - (n * tan_phi * ds / r) * (0.5 - ds / 24.0 * (
                    5.0 + 3.0 * t + 10.0 * c - 4.0 * cs - 9.0 * m_Esp[srctype] - ds / 30.0 * (
                        61.0 + 90.0 * t + 298.0 * c + 45.0 * ts - 252.0 * m_Esp[srctype] - 3.0 * cs))))
        out_pt.setX(m_arLonCenter[srctype] + (d * (1.0 - ds / 6.0 * (1.0 + 2.0 * t + c - ds / 20.0 * (
                    5.0 - 2.0 * c + 28.0 * t - 3.0 * cs + 8.0 * m_Esp[srctype] + 24.0 * ts))) / cos_phi))
    else:
...
if __name__ == "__main__":
    ...
    pt = GeoPoint(200305.31, 547459.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(202285.31, 544489.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(203095.31, 545209.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(204175.31, 545119.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(205255.31, 544219.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(206335.31, 546919.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(207505.31, 542239.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(209035.31, 542149.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())
    pt = GeoPoint(211465.31, 540799.91)
    output = convert(TM, GEO, pt)
    print(output.getX(), output.getY())